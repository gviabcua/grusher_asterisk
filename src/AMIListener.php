...
class AMIListener
{
    ...
    public function start($autoReconnect = true){
        ...
        $worker->onWorkerStart = function () use ($autoReconnect) {
            ...
            // обробник для всяких помилок конекта
            $ws_connection->onError = function (TcpConnection $connection, $code, $msg) use ($autoReconnect, $ws_connection) {
                echo "Connection error: $code - $msg\n";
                if ($autoReconnect) {
                    // Пауза 5 секунд перед retry через Timer (замість sleep, бо async і sleep морозить поток)
                    Timer::add(5000, function () use ($ws_connection) {
            ...
            $ws_connection->onClose = function (TcpConnection $connection) use ($autoReconnect, $ws_connection) {
                echo "Connection closed\n";
                if ($autoReconnect) {
                    // Worker::reloadAllWorkers();
                    // Retry через 5 секунд через Timer замість Worker::reloadAllWorkers()
                    Timer::add(5000, function () use ($ws_connection) {
                        $ws_connection->connect();
                    }, null, false); // Не повторювати Timer
                }
            };
            $ws_connection->connect();
        };
